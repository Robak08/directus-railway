...
export async function readRelationRow(
	...
	const fieldRow = (await database
		.select(['foreign_key_table', 'foreign_key_column'])
	...
	return {
		...
		foreign_key_table: fieldRow?.foreign_key_table ?? null,
		foreign_key_column: fieldRow?.foreign_key_column ?? null
	};
}
